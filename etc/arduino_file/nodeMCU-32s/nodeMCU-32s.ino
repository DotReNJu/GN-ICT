...
//"6MySQL 연동"
IPAddress server_addr(180, 83, 98, 139); // IP of the MySQL server here
char userSQL[] = "SQL아이디";                 // MySQL user login username
char passwordSQL[] = "SQL비밀번호";             // MySQL user login password
char query1[] = "SELECT * FROM 테이블상위명.공기청정기테이블명";
String query2;
char queryC2[256];
...
void _5_Network()
{
    ...
    int tmp_cnt=0;
    while (WiFi.status() != WL_CONNECTED&&tmp_cnt<=20)
    {
        //tmp_cnt+=1;
        delay(500);
        Serial.print(".");
...
void _6_MySQL()
{
    ...
    conn.connect(server_addr, 3306, userSQL, passwordSQL);
       Serial.println("suiuuuu");
    row_values *row = NULL;
    int setONOFF = 1;
    delay(1000);
    MySQL_Cursor *cur_mem = new MySQL_Cursor(&conn);
    
    Serial.println("goooo123321o");
    cur_mem->execute(query1);
     Serial.println("goooo12332564564561o");
    ...
    do
    {
        ...
        if (row != NULL)
        {
            setONOFF = atoi(row->values[4]);
            penStatus = penStatus == setONOFF ? penStatus : setONOFF;
        }
    } while (row != NULL);
    query2 = "UPDATE 테이블상위명.공기청정기테이블명 SET ";
    query2 += " misae=";
